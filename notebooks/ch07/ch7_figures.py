...
e=endemic(c_I=8); Ss,Is,Rs=e['S'],e['I'],e['R']
...
sol=simulate(1000,c_I=8); t=np.linspace(0,1000,4000); S,I,R=sol.sol(t)
...
sol=simulate(40150,c_I=8,n=70000); t=np.linspace(0,40150,24000); S,I,R=sol.sol(t); yr=t/365
...
t=np.linspace(0,600,3000); I1=simulate(600,c_I=8).sol(t)[1]; I2=simulate(600,c_I=4).sol(t)[1]
...
a1.set_title('Scenario 1:  $c_I=8$, $\\mathcal{R}_0\\approx1.92$  (supercritical outbreak)',fontsize=10.5)
...
a2.set_title('Scenario 2:  $c_I=4$, $\\mathcal{R}_0\\approx0.96$  (subcritical die-out)',fontsize=10.5)
for a in(a1,a2):
    for s in('top','right'):a.spines[s].set_visible(False)
time_arrow(a2)
...
cS,cI,cR,beta,gR,nu=10.,8.,10.,0.02,1/12,1/7300   # baseline parameters for the R_e computation below

# 5) Re(t): (a) first 600 d with I(t) overlaid, (b) ~110 yr long-term (Zhuolin Set 2)
fig,(axS,axL)=plt.subplots(1,2,figsize=(9.8,4.0))
sol=simulate(600,c_I=8); t=np.linspace(0,600,3000); S,I,R=sol.sol(t)
...
solL=simulate(40150,c_I=8,n=70000); tL=np.linspace(0,40150,24000); SL,IL,RL=solL.sol(tL); yr=tL/365
